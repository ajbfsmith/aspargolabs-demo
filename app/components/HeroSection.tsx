...
const qaData = [
  {
    ...
    answer:
      "HEZKUE® achieves detectable plasma sildenafil concentrations within 5 minutes of application. In a 56-subject crossover study, the comparator tablet showed zero measurable absorption at the same timepoint. Most users report noticeable effects within 5–10 minutes.",
  },
  {
    question: "Can I use it even if I've drunk or eaten recently?",
    answer:
      "Yes. Because HEZKUE® is absorbed through the buccal mucosa (inside the mouth), it bypasses the GI tract entirely. A high-fat meal that would delay a tablet by over an hour has no meaningful effect on spray absorption or onset time.",
  },
  {
    question: "How long does it work for?",
    answer:
      "HEZKUE® provides the same duration of action as standard sildenafil tablets — typically 4 to 6 hours. Total drug exposure (AUC) is bioequivalent to the reference tablet, so the therapeutic window is unchanged.",
  },
  {
    question: "Is it safe to use?",
    answer:
      "HEZKUE® contains the same active ingredient (sildenafil citrate) used in over 30 years of clinical practice. The spray formulation was evaluated in a full pharmacokinetic study with 56 healthy volunteers. Safety and tolerability profiles are consistent with established sildenafil data.",
  },
  {
    question: "How do I use this spray?",
    answer:
      "Shake gently, point the nozzle toward the inside of your cheek, and press the pump. Each actuation delivers a precisely calibrated dose. Hold for a few seconds before swallowing. The mint-flavoured suspension provides immediate sensory feedback.",
  },
  {
    question: "Is it better than Cialis or Viagra?",
    answer:
      "HEZKUE® delivers sildenafil — the same active ingredient as Viagra — but via a faster-absorbing oral spray. Compared to Cialis (tadalafil), HEZKUE® offers a different mechanism with rapid onset rather than a longer duration window. The best choice depends on your lifestyle and physician guidance.",
  },
];
          ...
          <div className="flex-1 min-w-0">
            <p
              className={`font-dm text-[16px] leading-snug transition-colors duration-300 ${
                isActive ? "text-text-primary" : "text-text-primary/85"
              }`}
            >
              {question}
            </p>
...
function MobileHeroFaq({
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const cards = sectionRef.current?.querySelectorAll(".mobile-faq-card");
      if (cards?.length) {
        gsap.from(cards, {
          y: 40,
          opacity: 0,
...
export default function HeroSection() {
  ...
  const playOpenTransition = useCallback(
    (index: number) => {
      ...
      gsap.set(panel, { opacity: 0, x: PANEL_SLIDE, width: 0, pointerEvents: "none" });
      ...
      tl.to(
        ...
        .to(
          ...
          { width: "45%", duration: TRANSITION_DURATION, ease: TRANSITION_EASE },
          0
        )
        .to(
          panel,
  ...
  const playCloseTransition = useCallback(() => {
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        ...
        if (panel) gsap.set(panel, { x: PANEL_SLIDE, opacity: 0, width: 0, pointerEvents: "none" });
    ...
    tl.to(
      ...
      .to(
        videoSideRef.current,
        { width: "100%", duration: TRANSITION_DURATION, ease: TRANSITION_EASE },
        0
      )
      .to(
        cardsLayerRef.current,
      ...
      <div className="hidden md:flex relative z-10 w-full h-[100dvh] items-center justify-center">
        <div className="relative w-full h-full">
          ...
          >
            ...
            <div ref={questionsRef} className="absolute inset-0">
              <div
                ref={cardsLayerRef}
              ...
              >
                {qaData.map((qa, i) => (
                  <div
                    ...
                    onMouseLeave={() => {
                      if (selectedIndex === null && !isAnimating.current)
                        startOrbit();
                    }}
                  >
                    <OrbitCard
          ...
          >
            <div ref={answerContentRef} className="w-full max-w-[540px] pl-10 lg:pl-16">
              {selectedIndex !== null && (
                <>
                  <button
                    onClick={handleClose}
